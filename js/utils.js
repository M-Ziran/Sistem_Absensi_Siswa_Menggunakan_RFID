...
function renderSidebar(activeItem) {
   ...
   const menus = {
      // Admin: HANYA kelola data — TIDAK ada Sesi Absensi
      admin: [
...
// --- Helper: load daftar kelas dari Firebase node /kelas ---
function loadKelasOptions(selectEl, defaultLabel = '— Pilih Kelas —') {
   return db.ref('kelas').get().then(snap => {
      const data = snap.val() || {};
